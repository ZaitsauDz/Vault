...
final class AppleBooksParser: MetadataParser {
    func canParse(url: URL) -> Bool {
        guard let host = url.host?.lowercased() else { return false }
        return host.contains("books.apple.com")
    }
    
    func parse(url: URL) async throws -> ParsedMetadata {
        // Apple Books URLs can be parsed using OpenGraph
        return try await parseWithOpenGraph(url: url, sourceType: .appleBooks)
    }
    ...
    private func parseWithOpenGraph(url: URL, sourceType: SourceType) async throws -> ParsedMetadata {
        ...
        let title = extractOpenGraphContent(from: html, property: "og:title") ?? "Apple Books"
